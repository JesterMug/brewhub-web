        ...
        if ($this->request->is('post')) {
            ...
            if (isset($responseKeys['success']) && $responseKeys['success'] == true) {
                ...
                if ($this->Forms->save($form)) {
                    ...
                    // Send emails (user + admin) but don't block UX on failures
                    try {
                        /** @var \App\Mailer\ContactMailer $mailer */
                        $mailer = new \App\Mailer\ContactMailer('default');
                        if (!empty($payload['email'])) {
                            $mailer->sendUserConfirmation($payload['email'], ['data' => $payload]);
                        }
                        $mailer->sendAdminNotification('superuser@brewhub.com', ['data' => $payload], $payload['email'] ?: null);
                    } catch (\Throwable $e) {
                        $this->Flash->error(__('Email failed.'));
                    }
